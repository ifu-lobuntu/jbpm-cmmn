...
public class CMMNServiceImpl implements CMMNService {
    ...
    public void transitionPlanItem(final long processInstanceId, final long uniqueId, final PlanItemTransition t) {
        ksession.execute(new AbstractPlanningCommand<Void>(processInstanceId, null) {
            ...
            public Void execute() {
                PlanItemInstance<?> planItem = getPlanItem(uniqueId);
                t.invokeOn(planItem);
                return null;
            }
        });
    ...
    @Override
    public void submitPlan(long processInstanceId, long planningTableContainerId, Collection<PlannableItem> plannedTasks, boolean resume) {
        ksession.execute(new SubmitPlanCommand(plannedTasks, processInstanceId,planningTableContainerId,resume));
    }

    @Override
    public Plan preparePlannableItem(long processInstanceId, long planningTableContainerId, String discretionaryItemId) {
        return null;
    }

    @Override
    public void makeDiscretionaryItemAvailable(long processInstanceId, long planningTableContainerId, String discretionaryItemId) {
